...
public:
    ...
    vector<Song> searchByArtist(const string artist) override{ //suprascriem functia virtuala din clasa Searchable
        vector<Song> result; //situatia in care vectorul result va fi sters la iesirea din functie cu destructorul din cls de baza
        for (const auto& song: songs){
...
static void printAlbumDetails(const unique_ptr<Media>& media){ 
    Album* album = dynamic_cast<Album*>(media.get()); //downcast la clasa Album
...
class GenreAlbum : public Album{ //clasa album de gen, mostenire publica
    set<string> genres; //genurile albumului 

public:
    GenreAlbum(string t, Artist a, int nr, vector<Song> s, int y, const set<string>& g) : Album(t, a, nr, s, y), genres(g){} //constructor initializare pt clasa de baza si genre

    void setGenres(const set<string>& g){ //setter gen
        genres=g;
    }

    const set<string>& getGenres() const{//getter genuri
        return genres;
    }
...
ostream & operator << (ostream &out, const GenreAlbum &ga){ //suprascriere << pentru GenreAlbum
    ...
    out << "Genres: "; //<< ga.getGenre() << endl; //adaug genre 
    for (const string& genre : ga.getGenres()){
        out<<genre<<" "; //afisam fiecare gen
    }
    out<<'\n';
    return out;
}
...
//partea a 3-a
template<class T> //clasa template pentru a putea folosi orice tip de date, adaugam functia play
class Track{
    T data;
...
template<typename T>
void printInfo(const T& data){ //functie printInfo afiseaza tipul de date
    cout<<"Information about data provided: "<<typeid(T).name();
    cout<<"\n";
}

template<>
void Track<Song>::play() const{ //specializare pentru Song
    cout<<"Playing: "<<data.getSongTitle()<<" by "<<data.getSongArtist().getArtistNickname()<<"\n";
}

template<>
void Track<Album>::play() const{ //specializare pentru Album
    cout<<"Playing album: "<<data.getAlbumTitle()<<" by "<<data.getAlbumArtist().getArtistNickname()<<"\n";
}

template<>
void Track<Playlist>::play() const{ //specializare pentru Playlist
    cout<<"Playing playlist: "<<data.getPlaylistTitle()<<"\n";
}

class SongIterator{ //clasa iterator pentru a parcurge vectorul de melodii
...
//functii utilitare
//functie de cautare dupa artist folosind copy_if
vector<Song> getSongsByArtist(const std::vector<Song>& songs, const string& artist){
    vector<Song> result;
    copy_if(songs.begin(), songs.end(), back_inserter(result), [&](const Song& song){
        return song.getSongArtist().getArtistNickname() == artist;
    });
    return result;
}

//functie de sortare melodii din vector dupa durata, folosind sort
void sortSongsByDuration(vector<Song>& songs){
    sort(songs.begin(), songs.end(), [](const Song& s1, const Song& s2){
        return s1.getSongTime() < s2.getSongTime();
    });
}
...
try{
    ...
    //inlocuim cu unique_ptr ptr obisnuit
    vector<unique_ptr<Media>> media;
    media.push_back(make_unique<Song>(tw1));
    media.push_back(make_unique<Album>(album_the_weeknd));
    media.push_back(make_unique<Playlist>(muzica));
    ...
    //partea 3
    Track<Playlist> track1(muzica); //instantiez obiect de tip track cu playlist-ul muzica
    Track<Album> track2(album_the_weeknd); //instantiez obiect de tip track cu albumul album_the_weeknd
    Track<Song> track3(tz1); //instantiez obiect de tip track cu melodia tz1
 
    track1.play(); 
    track2.play();
    track3.play();
    Track<Mixtape> track4(mixtape1);
    track4.play(); //afiseaza Now playing ptc e data necunoscuta si dupa afiseaza numele si continutul

    printInfo(track3); //tipul de date, returneaza %Track%Song%
    ...
    //utilizare getSongsByArtist() functie utilitara(copy_if)
    cout<<"\nSearching songs by artist:\n";
    vector<Song> songsByTheWeeknd = getSongsByArtist(others, "The Weeknd");
    //afisare
    for(const Song& song:songsByTheWeeknd)
        cout<<song.getSongTitle()<<'\n';

    //folosim functia sort pt aranjarea melodiilor crescator dupa durata intr-un vector de mel
    cout<<"\nSorting songs by duration:\n";
    sortSongsByDuration(the_weeknd_songs1);
    //afisare
    for(const Song& song:the_weeknd_songs1){
        song.displayDetails();
        cout<<'\n';
    }

    //facem o library noua pt myUser ca sa folosim si shared_ptr
    vector<shared_ptr<Media>> library;
    shared_ptr<Media> albumptr = make_shared<Album>(album_tzanca_uraganu);
    shared_ptr<Media> playlistptr = make_shared<Playlist>(muzica);

    library.push_back(albumptr); //adaugam playlist-uri si albume in library noua
    library.push_back(playlistptr);

    myUser.addToLibrary(static_cast<Media*>(&album_tzanca_uraganu)); //adaugam elementele din library in biblioteca utilizatorului
    myUser.addToLibrary(static_cast<Media*>(&muzica));

    cout<<"\nAfisare biblioteca actualizata:\n";
    myUser.displayLibrary();
