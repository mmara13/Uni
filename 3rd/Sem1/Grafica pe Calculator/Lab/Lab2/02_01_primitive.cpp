...
void RenderFunction(void)
{
	...
	float r = static_cast<float>(rand()) / static_cast<float>(RAND_MAX); //val rand intre 0 si 1
	float g = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
	float b = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);

	//TRIMitem culoarea in shader - am schimbat cum se primeste culoarea ca sa transmitem cu nume

	glUniform3f(glGetUniformLocation(ProgramId, "color"), r, g, b);
	glEnable(GL_POINT_SMOOTH);
	glPointSize(20.0);
	codCol = 1;
	glUniform1i(codColLocation, codCol);
