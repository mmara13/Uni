...
int codCol;							//	Variabila ce determina schimbarea culorii pixelilor in shader;					//	Unghiul de rotire al patratului;
...
float i = 0.0, alpha = 0.0, step = 0.3, beta = 0.01, angle = 0;

//patratul va merge sus-jos constant la apasarea oricarei dintre sagetile sus jos, si daca acestea vor fi apasate iar, cubul va relua animatia din directia sagetii
// pt sageata sus, cubul va veni de sus in jos, pt sageata jos, cubul va veni de jos in sus
//schimba sensul animatiei sus/jos
void MoveUp(void)
{
	i = i + alpha;
	if (i > 350.0)			//	Se asigura deplasarea sus-jps pe Oy in limitele ecranului,
		alpha = -step;		//	mai intai efectuandu-se deplasarea din sus in jos;
	else if (i < -350.0)
		alpha = step;
	angle = angle - beta;	//	Se actualizeaza unghiul de rotatie constant (mentine orbitarea); -
	//patratul se va roti in timp ce se misca pe axa Oy
...
{
	i = i + alpha;
	if (i < -350.0)			//	Se asigura deplasarea stanga-dreapta pe Oy in limitele ecranului,
		alpha = step;		//	mai intai efectuandu-se deplasarea de jos in sus;
	else if (i > 350.0)
		alpha = -step;
	angle = angle + beta;	//	Se actualizeaza unghiul de rotatie constant (mentine orbitarea);
	glutPostRedisplay();	//	Forteza redesenarea scenei;
...
{
	switch (key)			//	Procesarea tastelor sus jos(sageti)
	{						//	duce la deplasarea patratului pe axa Oy;
	case GLUT_KEY_UP:
		alpha += 10;
		glutIdleFunc(MoveUp);
		break;
...
void RenderFunction(void)
{
	...
	matrTransl = glm::translate(glm::mat4(1.0f), glm::vec3(0.0, i, 0.0));		//	Se translateaza de-a lungul axei Ox;
