...
void RenderFunction(void)
{
	...
	// variatii pt modif cul
	glUniform3f(objectColorLoc,
		0.8f, 0.4f, 0.3f);

	//glUniform3f(lightColorLoc, 1.0f, 1.0f, 1.0f);
	//variatii pt light color in functie de timp
	float time = glutGet(GLUT_ELAPSED_TIME) / 1000.0f;
	glUniform3f(lightColorLoc,
		0.5f + 0.5f * sin(time),
		0.5f + 0.5f * sin(time + 2),
		0.5f + 0.5f * cos(time));
	
	//glUniform3f(lightPosLoc, 400.f, -400.f, 400.f);
	//variatii pt positie - asta creaza umbre intr un mod mai dinamic
	glUniform3f(lightPosLoc,
		400.0f * sin(glutGet(GLUT_ELAPSED_TIME) / 1000.0f),
		-400.0f,
		400.0f * cos(glutGet(GLUT_ELAPSED_TIME) / 1000.0f));


	//glUniform3f(viewPosLoc, Obsx, Obsy, Obsz);
	glUniform3f(viewPosLoc,
		Obsx + 50.0f * sin(glutGet(GLUT_ELAPSED_TIME) / 500.0f),
		Obsy, Obsz);
